...
brand_website() {
  ...
  if [ -d "$publican_brand_path" ]; then
    ...
    #cp -r "$publican_brand_path/ja-JP" "$tmp_brand_dir"
  else
...
build_and_publish_document() {
  ...
  exec_publican build --publish --formats "$formats" \
    --brand_dir="$QL_BRAND_DIR" --embedtoc --langs=all
  exec_publican install_book --site_config ${WEB_CFG_PATH} --lang=$QL_LANG
  exec_publican install_book --site_config ${WEB_CFG_PATH} --lang=ja-JP
  ...
  #if [ "$book_dir" = "$QL_PORTAL_DIR" ]; then
  #  carousel="$book_dir/ja-JP/carousel.html"
  #  [ -e "$carousel" ] && cp -f "$carousel" "$WEB_TOC_PATH"/ja-JP
  #fi
}
...
create_portal() {
  ...
  local root_file2="${QL_PORTAL_DIR}/ja-JP/${QL_PORTAL_NAME}.xml"
...
EOF
  ...
  sed -i -e '/include href/d; /\<index /d' "$root_file2"
  ...
  sed -i -e "s,%%title%%,$QL_WEBSITE_TITLE,g" "$root_file2"
...
create_product() {
  ...
  local root_file2="${name}/ja-JP/${name}.xml"
...
EOF
  ...
  sed -i -e "s,%%title%%,$product,g" "$root_file2"
...
update_site() {
  ...
  local site_css2=$QL_BRAND_DIR/ja-JP/css/site_overrides.css
  local colorbox_js2=$QL_BRAND_DIR/ja-JP/javascript/jquery.colorbox-min.js
