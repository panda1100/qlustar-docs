...
CFG_FILE="manage-docs-withJP.cfg"
...
publish_brand() {
  ...
  if [ -f $tmp_brand_dir/tmp_brand/en-US/First_Steps_Guide.xml  ]; then
    exec_publican build --formats=xml --langs=$QL_LANG2 --publish
  fi  
...
build_and_publish_document() {
  ...
  if [ -d $book_dir/ja_JP ]; then
    exec_publican build --publish --formats "$formats" \
      --brand_dir="$QL_BRAND_DIR" --embedtoc --langs $QL_LANG2
  fi
  exec_publican install_book --site_config ${WEB_CFG_PATH} --lang $QL_LANG
  if [ -d $tmp_brand_dir/ja_JP ]; then
    exec_publican install_book --site_config ${WEB_CFG_PATH} --lang $QL_LANG2
  fi
